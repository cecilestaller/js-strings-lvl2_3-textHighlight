...
function searchInput() {
    ...
    const inputSearch = document.body.querySelector("#search-input").value.trim();
    // console.log(inputSearch);

    if(inputSearch !== " ") {
        const articleInhalt = article.innerHTML;
        const highContent = articleInhalt.replace(new RegExp(inputSearch, 'gi'), '<span class="highlight">$&</span>');
        article.innerHTML = highContent;
    } else {
